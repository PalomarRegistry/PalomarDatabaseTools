...
def versions_in(delta: dict[str, Any]) -> tuple[list[str], list[str]]:
    """Return active additions and the withdrawals this event must re-prove.

    Entry rows decide which versions arrived. Render and evidence rows may
    accompany them, but may not introduce a version health would fail to ask
    about. A full release re-proves every current stable tombstone, including
    ...
    for row in delta["additions"]:
        ...
        if target not in version_set:
            raise ValueError(f"release addition {row['path']} has no matching entry addition")
...
def _record(path: pathlib.Path, target: str) -> bytes:
    ...
    if not isinstance(value, dict) or (value.get("id"), value.get("version")) != (
        ...
        raise ValueError(f"publication health target disagrees with its path: {path}")
    return raw
...
def _bundle_records(
    ...
    for target in sorted(set(targets)):
        ...
        if target in additions and not _matches_row(raw, additions[target]):
...
def prepare(
    ...
    *,
    ...
    _bundle_records(delta, entries, bundle_entries, versions + withdrawn)
...
def verify(
    ...
    *,
    ...
    versions, withdrawn = versions_in(delta)
    _verify_bundled_records(delta, entries, versions + withdrawn)
    return versions, withdrawn
    ...
    try:
        ...
        if args.command == "verify":
            versions, withdrawn = verify(
                args.delta, commit=args.commit, entries=args.entries
            )
